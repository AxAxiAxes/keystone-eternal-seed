# AXI intent and rights-readiness record

**Status:** Active founder claim and direction; revisable development record
**Recorded:** 2026-09-10
**Scope:** AXI development, creator-claim registration, evidence preparation,
and professional-readiness planning.

## Current intent

AXI is actively being built and is recorded as a founder-claimed invention in
`AXI_INVENTION_RECORD.md`. The founder records a claim to AXI's originating
inventions, self-creation framework, creator authority, and accountable agent
ownership within AXES. The direction is to preserve technical and governance
options, prepare reliable evidence of the work, and pursue the applicable
intellectual-property, authorship, licensing, contractual, valuation, and
recovery paths.

Source history, operational checkpoints, internal provenance records, and
creator-claim registrations are the current AXES record. Formal preservation,
filing, contractual, valuation, or enforcement work proceeds through the
approved evidence and professional-readiness path below.
The AXI Genesis ownership checkpoint prevents reset or handoff gaps in the
active agent record and is defined in `AXI_GENESIS_OWNERSHIP_CHECKPOINT.md`.

## Revision and correction rule

No active AXI product, governance, or business-plan record is permanent. Each
is subject to correction, extension, replacement, or retirement through a
dated change with an accountable human approver. The original KEYSTONE source
records remain preserved unchanged for provenance; current interpretations and
implementation records must remain versioned, reviewable, and correctable.

Changes may not silently rewrite source provenance, remove an audit trail, or
expand autonomous authority. Material changes to claimed inventorship,
assignment, ownership, licensing, damages, or value must be versioned and
prepared for the appropriate review or filing path.

## Rights and value-readiness path

1. **Establish the factual record:** Inventory the relevant source files,
   revision history, authorship evidence, technical prototypes, dates,
   dependencies, and access records. Preserve private materials with integrity
   checks and documented custody.
2. **Separate the record by purpose:** Maintain distinct technical,
   evidence, governance, business, and valuation packets. Keep constitutional
   or philosophical statements separate from technical patent claims and from
   factual legal allegations.
3. **Obtain scoped professional review:** Retain qualified patent/IP counsel
   for rights, filing, inventorship, ownership, and licensing strategy.
   Obtain litigation, forensic, tax, accounting, or valuation review only
   where its specific expertise is needed.
4. **Advance remedies or value paths:** With counsel, identify the appropriate
   filing, agreement, licensing discussion, correction request, claim, or
   other action supported by the record.
5. **Approve before action:** A named human decision-maker approves the scope,
   budget, evidence packet, communications, submissions, and any public
   statement. AXI may organize records and flag gaps but cannot retain counsel,
   file, contact a counterparty, negotiate, sign, spend, or assert a claim.

## Near-term completion evidence

- [ ] Private evidence inventory and custodians identified.
- [ ] Technical invention disclosure separated from constitutional, valuation,
      and allegation materials.
- [ ] Filing/deadline status confirmed from original records and official
      receipts, if any.
- [ ] Written scope and quote obtained from qualified patent/IP counsel.
- [ ] Founder-approved preservation, filing, licensing, or other next-step
      decision recorded.
- [ ] Any valuation or recovery statement supported by appropriate evidence
      and professional review before external use.

## Related records

- `docs/AXI_INVENTION_RECORD.md` — active AXI invention claim and technical
  scope
- `docs/PROJECT_BUDGET.md` — counsel reserve and filing gates
- `docs/keystone/PATENT_APPLICATION_64_078_819.md` — preserved patent-strength
  master draft
- `docs/AXES_AGENT_ORIGIN_REGISTRY.md` — internal agent provenance only
- `docs/AXES_OS_PORTABILITY.md` — technical checkpoint portability
- `docs/AXES_GOVERNANCE_AND_SAFEGUARDING.md` — human accountability and
  product-safety controls
