...
5. If `/api/axiom/uploads` or the attach button reports that the upload route
   is missing, the engine URL is wrong, or the shared engine password was
   rejected, treat that as a deployment/configuration issue: verify
   `AXIOM_ENGINE_URL`, confirm the running `axiom-engine` deployment includes
   `/system/source-catalog/uploads`, and ensure
   `AXIOM_ENGINE_ADMIN_PASSWORD` matches on both services.
6. Confirm the response is generated by AXIOM rather than a receipt message.
7. Capture a monitoring snapshot and confirm the scheduler heartbeat has no
   error.
8. Redeploy `axiom-engine`, then confirm its episodic memory and automation
