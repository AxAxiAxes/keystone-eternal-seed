...
versioned `memory.record`, `automation.noop`, and `monitoring.snapshot`
allowlist; it cannot deploy,
...
3. Send a chat message through `/axiom`. If the response says `AXIOM chat is
   not configured`, add `OPENAI_API_KEY` only to the private `axiom-engine`
   encrypted variables, then redeploy that service.
4. Confirm the response is generated by AXIOM rather than a receipt message.
5. Capture a monitoring snapshot and confirm the scheduler heartbeat has no
   error.
6. Redeploy `axiom-engine`, then confirm its episodic memory and automation
