...
The current private `axiom-engine` seeds five agent records: Memory Curator,
Project Memory Manager, Automation Executor, Automation Auditor, and
Operations Observer. Its
...
| --- | ---: | ---: | --- |
| Current foundation | 5 | 5 | Existing private memory, continuous-memory management, safe no-op, audit, monitoring-observer, Genesis/governance-readiness, recovery-backup, and coordinate-chain roles only. |
| Command Center pilot | 5 | At most 5 | Operate the current roles only after protected-console monitoring is working. |
...
The registry implements the KEYSTONE protocol's origin, lineage, creator
ownership claim, and accountability record. External filing, recognition, and
enforcement work is tracked separately in
`AXI_INTENT_AND_RIGHTS_READINESS.md`.
...
| 4 | Command Center | Project Memory Manager | Maintain approved, non-sensitive memory-layer references and append explicitly approved continuity events. | A human confirms accuracy and retention; no secrets, personal data, source replacement, unreviewed claims, or external action. |
