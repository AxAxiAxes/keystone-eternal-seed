...
| 15 | **Founder said a "customer service prompt" should already be loaded into persistent memory. A repo-wide search found no such document under that or any similar name.** Please re-supply the exact prompt/instruction text (paste it, or point to where it was originally given) so it can be committed to a permanent, correctly-named doc | Cannot honor an instruction that was never actually captured in a durable, cross-session record — sessions do not share memory unless something is committed to this repo; this is either a first-time capture gap or was given in a session whose output was never persisted here | This entry; see `docs/memory/2026-09-19l-customer-service-prompt-not-found.md` |
