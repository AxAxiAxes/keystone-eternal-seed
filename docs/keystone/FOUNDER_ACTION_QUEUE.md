...
| --- | --- | --- | --- |
| 0 | **URGENT — live outage:** public AXIOM chat (`xiiom.com/axiom`) is returning HTTP 502 "AXIOM chat is temporarily unavailable" on every request as of 2026-09-18 ~10 AM PDT. Check Railway `axiom-engine` logs, confirm `OPENAI_API_KEY` validity/quota, and confirm Railway account balance/plan has not lapsed | The flagship live product is completely non-functional for every visitor right now — this is actively worse than, and likely the same root cause as, item 1 below | `PRODUCTION_INCIDENT_2026_09_18_CHAT_502.md` |
