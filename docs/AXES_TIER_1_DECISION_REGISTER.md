...
| --- | --- | --- | --- | --- |
| P0 | Restore provider-backed production chat, if the founder chooses to operate it | **Confirmed 2026-09-12.** `lucid-flow` is definitively the live production project: its `axiom-freedom` service shows Source Repo `AxAxiAxes/keystone-eternal-seed`, branch `axaxiaxes-axiom-monorepo` (this repository's production branch), and lists **`xiiom.com` as an active custom domain** in Settings > Networking. Its sibling `axiom-engine` service is correctly private-only (no public domain, `axiom-engine.railway.internal`). The earlier two-project ambiguity is resolved: `handsome-motivation` (`spirited-victory` + `nodejs`) is unrelated/stray, not the production deployment. Chat is still failing as of the last live test (HTTP 503, `{"error":"AXIOM chat is not configured"}`) | Source inspection of `apps/axiom-engine/chat-service.js` and `apps/axiom-engine/index.js` (the 503 fires only when `OPENAI_API_KEY` is falsy at process startup; Railway variable rows look identical whether filled or blank). Screenshots of `lucid-flow`'s `axiom-freedom` service Settings > Networking (Source Repo `AxAxiAxes/keystone-eternal-seed`, branch `axaxiaxes-axiom-monorepo`, custom domain `xiiom.com` listed) and its `axiom-engine` service (no public domain, private `axiom-engine.railway.internal`) | Authorized Railway operator should now go directly to `lucid-flow` > `axiom-engine` > Variables, (1) click into `OPENAI_API_KEY` specifically (not just confirm it is listed) to see its actual value, (2) replace it with a real, valid OpenAI API key if blank or a placeholder, (3) explicitly redeploy that service from its Deployments tab, then (4) confirm chat succeeds. The project-identity question that previously complicated this is now resolved; requires authorized external Railway access and an actual OpenAI API key/credential — neither can be created, retrieved, or set from this repository; do not disclose or commit credentials |
