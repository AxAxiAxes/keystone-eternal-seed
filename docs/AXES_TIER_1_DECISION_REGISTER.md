...
| --- | --- | --- | --- | --- |
| P0 | Restore provider-backed production chat, if the founder chooses to operate it | **Root cause found and fixed in-repository 2026-09-12, superseding the earlier OpenAI hypothesis.** A founder-provided Railway deploy-log screenshot showed `axiom-freedom` crash-looping with `Error [ERR_HTTP_HEADERS_SENT]` at `server.js`, triggered by `AXIOM engine returned HTTP 404` for `/system/gravity-center`, `/automation/readiness`, and `/system/bead-passports`. Source inspection found a genuine, systemic bug: 20 route handlers called `res.writeHead(200, ...)` *before* `await invokeEngine(...)` had resolved (nested inside `res.end(JSON.stringify(await invokeEngine(...)))`); when the awaited call rejected, the `catch` block's own `res.writeHead(...)` threw `ERR_HTTP_HEADERS_SENT` because headers were already sent, and this second throw — inside an async callback passed directly to `http.createServer` — became an unhandled promise rejection that terminates the whole Node process by default (Node 20). Every symptom traces back to this one bug: the 404s (from an `axiom-engine` deployment older than `axiom-freedom`'s newer routes) triggered the crash, the crash took down chat (`/api/axiom`, same process), and the crash also produced the raw non-JSON "Request failed (502)" errors seen in the automation console (a mid-request crash drops the connection before any body is sent). The prior working theory — an OpenAI-side cause (bad key, no billing/quota, wrong model) — was never actually confirmed; it was an unverified guess made because the client-visible message is identical for an unreachable engine, a non-2xx engine response, and (as now understood) a crash-in-progress. All 20 call sites were restructured to `const result = await invokeEngine(...); res.writeHead(200, ...); res.end(JSON.stringify(result));`, matching the file's own already-correct convention used elsewhere. A new regression test drives every affected route against a simulated stale (404-returning) engine and asserts the server responds gracefully and stays alive for subsequent requests, rather than crashing the test process | Live `POST https://xiiom.com/api/axiom` (`action: "chat"`) test after this fix is merged and redeployed: expect either `200` (engine healthy) or a graceful non-crash-loop `502`/`404` (engine still stale/unavailable) — no more repeating `ERR_HTTP_HEADERS_SENT` crash-loop entries in Railway's `axiom-freedom` deploy logs. `apps/axiom-freedom/test/axiom-proxy.test.js` (`survives engine failures on every automation status route without crashing the process`) passing locally and in CI | Authorized Railway operator should confirm the redeploy completed, retest chat, and separately confirm `axiom-engine`'s own active-deployment/source-connection status in Railway (a stale/absent engine deployment returning 404s was the apparent *trigger*, even though this fix is what stops it from being catastrophic). Requires authorized external Railway account access; nothing further can be diagnosed or fixed from this repository alone |
